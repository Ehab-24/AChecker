...
class ForwardExplorerState(object):
    ...
    def next_states(self):
        ...
        for succ in possible_succs:
            ...
            if next_slices:
                next_states.append(ForwardExplorerState(
                    succ, self.path, branches, next_slices))
        return next_states
...
class ForwardExplorer(object):
    ...
    def find(self, slices, looplimit=2, avoid=frozenset(), prefix=None):
        ...
        if prefix is None:
            ...
            state = ForwardExplorerState(
                self.cfg._ins_at[prefix].bb, prefix, 0, slices)
        ...
        while not todo.empty():
            ...
            if any(is_substr(pth, state.path) for pth in self.blacklist):
                logging.info("BLACKLIST hit for %s" %
                             (', '.join('%x' % i for i in state.path)))
                continue
