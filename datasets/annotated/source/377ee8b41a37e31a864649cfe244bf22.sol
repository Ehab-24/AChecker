...
contract BetGame {
    ...
    constructor(uint t, string memory a, string memory b) public {
        owner = msg.sender;
        betEnd = t;
        teamA = a;
        teamB = b;
    ...
    function getInfoA(
        uint index
    ) public view onlyowner returns (address, uint) {
        ...
        if (totalA > 0) {
            ...
            for (uint p = 0; p < A.length; p++) {
                if (A[p].deposit > 0) {
                    if (
                        A[p].player.send(
                            A[p].deposit + (A[p].deposit / 1000000) * ratio
                        )
                    ) {
                        A[p].deposit = 0;
                    }
        ...
        if (totalB > 0) {
            ...
            for (uint p = 0; p < B.length; p++) {
                if (B[p].deposit > 0) {
                    if (
                        B[p].player.send(
                            B[p].deposit + (B[p].deposit / 1000000) * ratio
                        )
                    ) {
                        B[p].deposit = 0;
                    }
        ...
        if (totalD > 0) {
            ...
            for (uint p = 0; p < D.length; p++) {
                if (D[p].deposit > 0) {
                    if (
                        D[p].player.send(
                            D[p].deposit + (D[p].deposit / 1000000) * ratio
                        )
                    ) {
                        D[p].deposit = 0;
                    }
    ...
    function status(
        address addr
    ) public view returns (uint, uint, uint, uint, uint, uint, bool) {
        uint a;
        uint b;
        uint d;
    ...
    function betA() public payable {
        require(now <= betEnd, "Betting already ended.");

        require(open, "Game closed");

        require(
            msg.value >= 0.01 ether,
            "Single bet must be at least 0.01 ether"
        );
        totalA += msg.value;
